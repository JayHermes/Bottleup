    ...
    <div class="authGate">
      <div class="authCard">
        ...
        <form class="authForm" id="authForm">
          ...
          ${message ? `<div class="authMessage ${error ? 'authError' : ''}">${message}</div>` : ''}
        </form>
...
if (!supabase) {
  root.innerHTML = `<div class="authGate"><div class="authCard"><div class="authBrand"><div class="authLogo">♻</div><span>BottleUp</span></div><h1 class="authTitle">Supabase is not configured</h1><p class="authCopy">Add VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY to bottle-up-mvp/.env for local development, and add the same variables to your deployment environment.</p><div class="authMessage authConfig">No credentials were found by Vite.</div></div></div>`
} else {
